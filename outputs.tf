...
output "public_ec2_ssh_command" {
  ...
  value       = "ssh -i ${var.key_pair_name}.pem ec2-user@${aws_instance.ec2_public.public_ip}"
}
...
output "ssh_via_bastion_command" {
  ...
  value       = "ssh -i ${var.key_pair_name}.pem -J ec2-user@${aws_instance.ec2_public.public_ip} ec2-user@${aws_instance.ec2_private.private_ip}"
}
